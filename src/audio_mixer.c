...
static int AudioMixer_new(lua_State* L)
{
    ...
    if (!capi || !engine) {
        if (versionError) {
            ...
            return luaL_argerror(L, firstArg, "expected object that implements the Auproc C API");
        }
    }
    ...
    if (senderArg <= lastArg)
    {
        ...
        if (!senderCapi || !sender) {
            if (errReason == 1) {
                ...
                return luaL_argerror(L, senderArg, "expected sender object that implements the Sender C API");
            }
        }
