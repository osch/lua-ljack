# Activations script for add the ljack module to the lua path when
# building ljack using the Makefile.
# Source this into interactive shell by invoking ". activates.sh" from this directory
# This is not necessary if ljack is installed, e.g. via luarocks.

this_dir=$(pwd)

lualjack_dir=$(cd "$this_dir"/..; pwd)

if [ ! -e "$lualjack_dir/src/activate.sh" -o ! -e "$lualjack_dir/src/main.c" ]; then

    echo '**** ERROR: ". activate.sh" must be invoked from "src" directory ***'

else

    echo "Setting lua paths for: $lualjack_dir"

    add_lua_path="$lualjack_dir/src/?.lua;$lualjack_dir/src/?/init.lua"
    add_lua_cpath="$lualjack_dir/src/build"
...
unset lua_cmd this_dir lualjack_dir add_lua_path add_lua_cpath lua_version lua_path_vers vers default_version
