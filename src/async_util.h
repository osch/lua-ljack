...
#elif defined(LJACK_ASYNC_USE_WINTHREAD)
typedef DWORD ThreadId;
#elif defined (LJACK_ASYNC_USE_STDTHREAD)
...
#define async_lock_init ljack_async_lock_init
void async_lock_init(Lock* lock);

/* -------------------------------------------------------------------------------------------- */

#define async_lock_destruct ljack_async_lock_destruct
...
static inline bool async_lock_tryacquire(Lock* lock) 
{
#if defined(LJACK_ASYNC_USE_PTHREAD)

    int rc = pthread_mutex_trylock(&lock->lock);
    if (rc == 0 || rc == EBUSY) {
        return (rc == 0);
    } else {
...
#elif defined(LJACK_ASYNC_USE_WINTHREAD)
    return TryEnterCriticalSection(&lock->lock) != 0;
...
#define async_mutex_init ljack_async_mutex_init
void async_mutex_init(Mutex* mutex);

/* -------------------------------------------------------------------------------------------- */

#define async_mutex_destruct ljack_async_mutex_destruct
...
#define async_mutex_wait ljack_async_mutex_wait
void async_mutex_wait(Mutex* mutex);

/* -------------------------------------------------------------------------------------------- */

#define async_mutex_wait_millis ljack_async_mutex_wait_millis
